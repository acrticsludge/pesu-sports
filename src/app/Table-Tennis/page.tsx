...
export default function TableTennis() {
    ...
    async function fetchBookings() {
      try {
        ...
        const response = await fetch(
          `http://localhost:5000/api/bookings?sport=TableTennis&court=${encodeURIComponent(
            activeTab
          )}`
        );
  ...
  function Content({
    ...
    const handleBook = () => {
      ...
      router.push(
        `/Table-Tennis/Table-Tennis-Checkout?day=${encodeURIComponent(
          String(selectedSlot.day)
        )}&hour=${encodeURIComponent(
          String(selectedSlot.hour)
        )}&tab=${encodeURIComponent(activeTab)}&sport=TableTennis`
      );
    };
