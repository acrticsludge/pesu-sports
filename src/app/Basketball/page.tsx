...
export default function Basketball() {
    ...
    async function fetchBookings() {
      try {
        ...
        const response = await fetch(
          `http://localhost:5000/api/bookings?sport=Basketball&court=${encodeURIComponent(
            activeTab
          )}`
        );
  ...
  function Content({
    ...
    const handleBook = () => {
      ...
      router.push(
        `/Basketball/Basketball-Checkout?day=${encodeURIComponent(
          String(selectedSlot.day)
        )}&hour=${encodeURIComponent(
          String(selectedSlot.hour)
        )}&tab=${encodeURIComponent(activeTab)}&sport=Basketball`
      );
    };
